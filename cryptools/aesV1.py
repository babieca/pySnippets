...
class AES(object):
    ...
    # valid key sizes
    keySize = dict(SIZE_128=128, SIZE_192=192, SIZE_256=256)
    ...
    def expandKey(self, key, size, expandedKeySize):
        ...
        for j in range(int(size/byte_size)):
            expandedKey[j] = key[j]
        currentSize += int(size/byte_size)
        ...
        while currentSize < expandedKeySize:
            ...
            if currentSize % (size/byte_size) == 0:
                t = self.core(t, rconIteration)
                rconIteration += 1
            # For 256-bit keys, we add an extra sbox to the calculation
            if size == self.keySize["SIZE_256"] and ((currentSize % (size/byte_size)) == 16):
            ...
            # key.
            for m in range(4):
                expandedKey[currentSize] = expandedKey[currentSize - int(size/byte_size)] ^ t[m]
                currentSize += 1
    ...
    def encrypt(self, iput, key, size):
        ...
        #print('ExpandedKey: {:4}'.format(expandedKey))
        #print('\n'.join([''.join(['{:4}'.format(item) for item in row]) for row in expandedKey]))
...
def entropy(x):
    ...
    entr = -np.sum(p * safe_ln2(p), axis=1)
...
if __name__ == "__main__":
    ...
    data = data[data[:,1].argsort()]        # sort data by column 1
